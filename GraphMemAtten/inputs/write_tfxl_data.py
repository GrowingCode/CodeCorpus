...
def generate_tfxl_record(origin_filepath, record_filepath):
  ...
  for line in open(origin_filepath, 'r'):
    ...
    origin_sequence, relative_to_part_first, valid_mask, parent_hint, seq_part_skip, token_type, origin_sequence_exact = one_example.split("#")
    ...
    examples.append((int_origin_sequence, int_relative_to_part_first, int_valid_mask, int_parent_hint, int_seq_part_skip, int_token_type, int_origin_sequence_exact))
    ...
    assert len(int_parent_hint) == len(int_seq_part_skip)
...
def handle_examples(examples, example_max_ele_size, writer):
  ...
  for example in examples:
    ...
    np_batch_seq_part_skip = np.concatenate([np_batch_seq_part_skip, np.expand_dims(pad_vector_to_specified_length(example[4], example_max_ele_size), axis=1)], axis=1)
    np_batch_token_type = np.concatenate([np_batch_token_type, np.expand_dims(pad_vector_to_specified_length(example[5], example_max_ele_size), axis=1)], axis=1)
    np_batch_origin_sequence_exact = np.concatenate([np_batch_origin_sequence_exact, np.expand_dims(pad_vector_to_specified_length(example[6], example_max_ele_size), axis=1)], axis=1)
