...
#     self.multi_loss_calculator = MultiLossCalculator()
  ...
  def multi_decode(self, dec_inp, target, relative_to_part_first, all_outputs, mems, valid_mask, type_hint, parent_hint, position_hint, is_training):
...
#     assert b_size.numpy() <= batch_size
    ...
    outputs, _, _, _, new_mems = self.transformer_model.transformer(dec_inp, target, mems, valid_mask, type_hint, parent_hint, position_hint, is_training=is_training, calculate_loss=False)
...
#     transferred_outputs = used_outputs
    ...
    probs, predictions, loss = self.loss_calculator.mask_adaptive_logsoftmax(transferred_outputs, target, valid_mask, type_hint, parent_hint, position_hint, is_training == False, multi_infer_train_to_predict_unk)
