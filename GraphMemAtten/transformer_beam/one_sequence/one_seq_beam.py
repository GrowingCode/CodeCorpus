...
from utils.meta_util import get_one_layer_varied_memory_shape_in_while_loop
...
class OneSeqBeam:
  ...
  def __call__(self, whole_seq, skt_token_split):
  ...
  def infer(self, last_token, mems_before_last, steps):
    ...
    def infer_body(i, i_len, probs, ens, l_token, *mems_tuple):
      ...
      probs, predictions, _, new_mems = transformer(l_token, -1, mems, is_training=0, mem_len=oracle_mem_len)
    ...
    mems_shapes = get_one_layer_varied_memory_shape_in_while_loop()
