...
#     print("tf.shape(self.l_token):" + str(tf.shape(self.l_token)))
  ...
  def infer_one_step(self):
    self.i += 1
    output, _, _, _, new_mems = self.transformer_model.transformer(self.l_token, tf.zeros_like(self.l_token)-1, self.mems, tf.ones_like(self.l_token), tf.ones_like(self.l_token), is_training=False, calculate_loss=False)
...
class OneStepMultiInfer():
  ...
  def __init__(self, transformer_model, multi_decode_model, mems_before_last, last_token):
    ...
    output, _, _, _, _ = self.transformer_model.transformer(last_token, tf.zeros_like(last_token)-1, mems_before_last, tf.ones_like(last_token), tf.ones_like(last_token), is_training=0, calculate_loss=False)
    self.output = output
    self.i = -1
...
def framework_token_infer(inferrer, parent_hint, steps):
  ''' here mems_before_last shape must be [n_layer memory_length 1 feature_size] '''
#     all_mems = update_recent_fixed_length_memory(mems_before_last, old_new_mems)
  ''' output shape should be [predict_length batch_size feature_size] '''
  ''' output shape should be [1 1 feature_size] '''
