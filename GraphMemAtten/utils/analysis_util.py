...
def get_first_param_right_but_second_param_wrong(first_infer, first_oracle, second_infer, second_oracle):
  pass
...
def statistic_acc(a_infer, a_oracle):
  ...
  right_pe_wrong_dict = {}
  wrong_pe_wrong_dict = {}
  for ai, ao in zip(a_infer, a_oracle):
    assert len(ai) == len(ao)
      ...
      if islist:
        ...
        for i in range(sao_size):
          ...
          if i < sai_size:
            a_en = sep_ai[i]
            if a_en == o_en:
              right_dict[o_en_str] = right_dict.get(o_en_str, default=0) + 1
              if list_right:
                pe_dict = right_pe_right_dict.get(o_en_str, default={})
                pe_dict[list_str] = pe_dict.get(list_str, default=0) + 1
                right_pe_right_dict[o_en_str] = pe_dict
              else:
                pe_dict = right_pe_wrong_dict.get(o_en_str, default={})
                pe_dict[list_str] = pe_dict.get(list_str, default=0) + 1
                right_pe_wrong_dict[o_en_str] = pe_dict
            else:
              wrong_dict[o_en_str] = wrong_dict.get(o_en_str, default=0) + 1
              pe_dict = wrong_pe_wrong_dict.get(o_en_str, default={})
              pe_dict[list_str] = pe_dict.get(list_str, default=0) + 1
...
#           pass
#         else:
#           wrong_pe_dict[list_str] = wrong_pe_dict.get(list_str, default=0) + 1
      else:
